class MitadTutorial extends Phaser.Scene {
    constructor() {
        super("MitadTutorial");
    }

    create() {
        this.fondoPausa = this.add.image(width / 2, height / 2, 'fondoPausa');
		this.tablonPausa = this.add.image(width / 2, height / 2, 'tablonPausa');
		
		var texto;
		if(idioma.idioma.includes("es")){
			texto = "¡Parece que ya le vas cogiendo el tranquillo a esto! Bien, ahora sólo te queda encontrar el resto de objetos y finalizar el paquete. Pero recuerda que cuantas más cosas lleves encima, más lento te moverás por las oficinas de Dielivery. ¡El tiempo apremia!";
		}else{
			texto = "It seems that you are starting to get the hang of it! Well, now you have to find some offerings and finish your package. But remeber: if you carry a lot of things, you will move slower. Time runs out, hurry!";
		}
		var config = {
			fontFamily: 'Sylfaen',
			fontSize: '18px',
			color: '#fff',
			stroke: '#000000',
			strokeThickness: 0.5,
			align: 'justify',  // 'left'|'center'|'right'|'justify'
			maxLines: 0,
			lineSpacing: 0,
			fixedWidth: 298,
			fixedHeight: 300,
			rtl: false,
			testString: '|MÉqgy',
			wordWrap: {
				width: 298,
				callback: null,
				callbackScope: null,
				useAdvancedWrap: false
			},
			metrics: false
			// metrics: {
			//     ascent: 0,
			//     descent: 0,
			//     fontSize: 0
			// }
		};
		
		this.textoFinal = this.add.text(width / 2 - 149, height / 2 - 150, texto, config);
		
        this.confirmar = this.add.image(width / 2, height / 2 + separacionBotones, 'continuar').setInteractive();
		this.confirmar.on('pointerdown', () => {this.confirmar.setTexture("continuar_pulsado");});
        this.confirmar.on('pointerup', () => this.ReanudarJuego());
        this.confirmar.on('pointerover', () => {if(this.input.activePointer.isDown){this.confirmar.setTexture("continuar_pulsado");}});
        this.confirmar.on('pointerout', () => {this.confirmar.setTexture("continuar");});
    }
	
	ReanudarJuego(){
		this.scene.resume("Game");
		this.scene.stop("MitadTutorial");
	
		
	}
}
